...
import { IconPlus, IconSearch, IconEdit, IconTrash, IconSend, IconBell, IconBuildingCommunity, IconUsers } from "@tabler/icons-react";
...
export default function Announcements() {
  ...
  const tabFiltered = tab === "published" ? items.filter((a) => a.published)
  ...
  return (
    <Box>
      ...
      <Group mb="md" gap="sm">
        <TextInput placeholder="Search announcements..."
          leftSection={<IconSearch size={14}/>} value={search}
          onChange={(e) => setSearch(e.target.value)} style={{ flex: 1, maxWidth: 360 }} size="sm" />
      </Group>
      ...
      <Modal opened={modalOpen} onClose={() => setModalOpen(false)}
        title={editTarget ? "Edit Announcement" : "New Announcement"} size="lg">
        <Box style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <TextInput label="Title" placeholder="Announcement title" value={form.title}
            onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} required />
          <Textarea label="Body" placeholder="Write the full announcement message..." value={form.body}
            onChange={(e) => setForm((f) => ({ ...f, body: e.target.value }))} minRows={4} required />
          <SimpleGrid cols={2} spacing="sm">
            <Select label="Target Audience" value={form.target}
