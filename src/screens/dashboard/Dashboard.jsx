...
import {
  Users, UserCheck, UserMinus, Clock, TrendingUp, Wallet,
  Briefcase, Calendar, Bell, ArrowUpRight, ArrowDownRight,
...
import { SPACING, GAP, LAYOUT, PADDING } from "../../theme/spacing";
...
const MONTHLY_HEADCOUNT = [
  { month: "Jan", employees: 62, joiners: 4, leavers: 1 },
  { month: "Feb", employees: 65, joiners: 5, leavers: 2 },
  { month: "Mar", employees: 68, joiners: 6, leavers: 3 },
  { month: "Apr", employees: 71, joiners: 5, leavers: 2 },
  { month: "May", employees: 74, joiners: 6, leavers: 3 },
  { month: "Jun", employees: 77, joiners: 5, leavers: 2 },
  { month: "Jul", employees: 80, joiners: 6, leavers: 3 },
  { month: "Aug", employees: 83, joiners: 5, leavers: 2 },
  { month: "Sep", employees: 86, joiners: 6, leavers: 3 },
  { month: "Oct", employees: 88, joiners: 5, leavers: 3 },
  { month: "Nov", employees: 91, joiners: 6, leavers: 3 },
  { month: "Dec", employees: 94, joiners: 6, leavers: 3 },
];
...
const RECENT_ACTIVITY = [
  { id: 1, name: "Arjun Kumar",   action: "Joined onboarding",         time: "2h ago",  type: "join",    dept: "IT"         },
  { id: 2, name: "Safeer",        action: "Leave request approved",     time: "3h ago",  type: "leave",   dept: "Finance"    },
  { id: 3, name: "Priya Sharma",  action: "New hire added",             time: "5h ago",  type: "join",    dept: "HR"         },
  { id: 4, name: "Suriya",        action: "Performance review due",     time: "1d ago",  type: "alert",   dept: "IT"         },
  { id: 5, name: "Mani",          action: "Check-in recorded",          time: "1d ago",  type: "attend",  dept: "IT"         },
  { id: 6, name: "Divya Nair",    action: "Onboarding completed",       time: "2d ago",  type: "complete",dept: "IT"         },
];
...
const SectionTitle = ({ title, sub, action, onAction }) => (
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: SPACING[4] }}>
    <div>
      <p style={{ margin: 0, fontSize: FONT_SIZE.md, fontWeight: FONT_WEIGHT.bold, color: COLORS.textLight }}>{title}</p>
      {sub && <p style={{ margin: "2px 0 0", fontSize: FONT_SIZE.xs, color: COLORS.textMutedLight }}>{sub}</p>}
    </div>
    {action && (
      <button onClick={onAction} style={{ display: "flex", alignItems: "center", gap: 3, fontSize: FONT_SIZE.xs, color: COLORS.primary, background: "none", border: "none", cursor: "pointer", fontFamily: FONT_FAMILY.base, fontWeight: FONT_WEIGHT.medium }}>
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: COLORS.surfaceLight, border: `1px solid ${COLORS.borderLight}`, borderRadius: RADIUS.lg, padding: `${GAP.sm}px ${GAP.md}px`, boxShadow: SHADOW.card, fontSize: FONT_SIZE.xs, fontFamily: FONT_FAMILY.base }}>
      <p style={{ margin: `0 0 ${GAP.xs}px`, fontWeight: FONT_WEIGHT.semibold, color: COLORS.textLight }}>{label}</p>
      {payload.map((p) => (
  ...
  return (
    <div style={{ fontFamily: FONT_FAMILY.base }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: SPACING[5], flexWrap: "wrap", gap: GAP.md }}>
        <div>
          <h1 style={{ margin: 0, fontSize: FONT_SIZE["2xl"], fontWeight: FONT_WEIGHT.bold, color: COLORS.textLight }}>Dashboard</h1>
          <p style={{ margin: `${GAP.xs}px 0 0`, fontSize: FONT_SIZE.base, color: COLORS.textMutedLight }}>{today}</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: GAP.sm }}>
          <div style={{ position: "relative" }}>
            <div style={{ width: 38, height: 38, borderRadius: RADIUS.full, background: COLORS.gray50, border: `1px solid ${COLORS.borderLight}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
              <Bell size={17} color={COLORS.gray700} />
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: GAP.md, marginBottom: SPACING[5] }}>
        {kpis.map((k) => (
          <div key={k.label} style={{ background: COLORS.surfaceLight, borderRadius: RADIUS["2xl"], border: `1px solid ${COLORS.borderLight}`, boxShadow: SHADOW.card, padding: `${SPACING[4]}px ${SPACING[4]}px` }}>
            ...
            <p style={{ margin: 0, fontSize: FONT_SIZE["2xl"], fontWeight: FONT_WEIGHT.bold, color: COLORS.textLight, lineHeight: 1 }}>{k.value}</p>
            <p style={{ margin: "4px 0 1px", fontSize: FONT_SIZE.xs, fontWeight: FONT_WEIGHT.medium, color: COLORS.textLight }}>{k.label}</p>
            <p style={{ margin: 0, fontSize: FONT_SIZE.xs, color: COLORS.textMutedLight }}>{k.sub}</p>
          </div>
        ))}
      </div>

      {/* ── ROW 1: Headcount Trend + Attendance Pie + Today Breakdown ── */}
      <div style={{ display: "grid", gridTemplateColumns: "1.8fr 1fr 1fr", gap: GAP.md, marginBottom: GAP.md }}>
        ...
        <CardBox>
          ...
          <ResponsiveContainer width="100%" height={210}>
            <AreaChart data={MONTHLY_HEADCOUNT}>
              ...
              <XAxis dataKey="month" tick={{ fontSize: FONT_SIZE.xs, fill: COLORS.textMutedLight }} />
              <YAxis tick={{ fontSize: FONT_SIZE.xs, fill: COLORS.textMutedLight }} />
            ...
            {attendancePie.map((d) => (
              <div key={d.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: GAP.xs }}>
                  ...
                  <span style={{ fontSize: FONT_SIZE.xs, color: COLORS.textMutedLight }}>{d.name}</span>
                </div>
                <span style={{ fontSize: FONT_SIZE.xs, fontWeight: FONT_WEIGHT.bold, color: COLORS.textLight }}>{d.value}</span>
              </div>
            ))}
          </div>
        </CardBox>
        ...
        <CardBox style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ padding: `${SPACING[4]}px ${SPACING[4]}px ${SPACING[3]}px`, borderBottom: `1px solid ${COLORS.borderLight}` }}>
            <p style={{ margin: 0, fontSize: FONT_SIZE.md, fontWeight: FONT_WEIGHT.bold, color: COLORS.textLight }}>Today's Numbers</p>
            <p style={{ margin: "2px 0 0", fontSize: FONT_SIZE.xs, color: COLORS.textMutedLight }}>{today.split(",")[0]}</p>
          </div>
          {[
            ...
            <div key={label} style={{ padding: `${SPACING[3]}px ${SPACING[4]}px`, borderBottom: i < arr.length - 1 ? `1px solid ${COLORS.borderLight}` : "none" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: pct !== null ? 5 : 0 }}>
                <span style={{ fontSize: FONT_SIZE.sm, color: COLORS.textMutedLight }}>{label}</span>
                <span style={{ fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.bold, color }}>{value}{pct !== null ? ` (${pct}%)` : ""}</span>
              </div>
              {pct !== null && (
                <div style={{ height: 4, background: COLORS.borderLight, borderRadius: RADIUS.full, overflow: "hidden" }}>
        ...
        <CardBox>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={ATTENDANCE_WEEK} barSize={18} barGap={4}>
              <CartesianGrid strokeDasharray="3 3" stroke={COLORS.borderLight} />
              <XAxis dataKey="day" tick={{ fontSize: FONT_SIZE.xs, fill: COLORS.textMutedLight }} />
              <YAxis tick={{ fontSize: FONT_SIZE.xs, fill: COLORS.textMutedLight }} />
              <Tooltip content={<CustomTooltip />} />
              <Legend wrapperStyle={{ fontSize: FONT_SIZE.xs, fontFamily: FONT_FAMILY.base }} />
              ...
              return (
                <div key={d.name}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: FONT_SIZE.sm, color: COLORS.textLight, fontWeight: FONT_WEIGHT.medium }}>{d.name}</span>
                    <span style={{ fontSize: FONT_SIZE.xs, color: COLORS.textMutedLight }}>{count} <span style={{ color: COLORS.gray400 }}>({pct}%)</span></span>
                  </div>
                  <div style={{ height: 7, background: COLORS.borderLight, borderRadius: RADIUS.full, overflow: "hidden" }}>
      ...
      {/* ── ROW 3: Payroll Trend + Leave Types + Top Performers ── */}
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: GAP.md, marginBottom: GAP.md }}>
        ...
        <CardBox>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={PAYROLL_MONTHS}>
              <CartesianGrid strokeDasharray="3 3" stroke={COLORS.borderLight} />
              <XAxis dataKey="month" tick={{ fontSize: FONT_SIZE.xs, fill: COLORS.textMutedLight }} />
              <YAxis tickFormatter={fmtINR} tick={{ fontSize: FONT_SIZE.xs, fill: COLORS.textMutedLight }} />
              <Tooltip formatter={(v) => [`₹${v.toLocaleString("en-IN")}`, "Payroll"]} content={<CustomTooltip />} />
        ...
        <CardBox style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ padding: `${SPACING[4]}px ${SPACING[4]}px ${SPACING[3]}px`, borderBottom: `1px solid ${COLORS.borderLight}` }}>
            <p style={{ margin: 0, fontSize: FONT_SIZE.md, fontWeight: FONT_WEIGHT.bold, color: COLORS.textLight }}>Top Performers</p>
            <p style={{ margin: "2px 0 0", fontSize: FONT_SIZE.xs, color: COLORS.textMutedLight }}>Q2 2026 leaderboard</p>
          </div>
          {TOP_PERFORMERS.map((p, i, arr) => {
            const av = getAvatarColor(p.name);
            return (
              <div key={p.name} style={{ display: "flex", alignItems: "center", gap: GAP.sm, padding: `${SPACING[3]}px ${SPACING[4]}px`, borderBottom: i < arr.length - 1 ? `1px solid ${COLORS.borderLight}` : "none" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ margin: 0, fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.semibold, color: COLORS.textLight, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</p>
                  <p style={{ margin: 0, fontSize: FONT_SIZE.xs, color: COLORS.textMutedLight }}>{p.badge}</p>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <p style={{ margin: 0, fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.bold, color: COLORS.success }}>{p.score}</p>
                  <p style={{ margin: 0, fontSize: FONT_SIZE.xs, color: COLORS.textMutedLight }}>score</p>
                </div>
              </div>
            );
          })}
        ...
        <CardBox>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: GAP.sm }}>
            {RECENT_ACTIVITY.map((a) => {
              ...
              return (
                <div key={a.id} style={{ display: "flex", alignItems: "center", gap: GAP.sm, padding: `${GAP.sm}px 0`, borderBottom: `1px solid ${COLORS.borderLight}` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.semibold, color: COLORS.textLight, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.name}</p>
                    <p style={{ margin: 0, fontSize: FONT_SIZE.xs, color: COLORS.textMutedLight }}>{a.action}</p>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3, flexShrink: 0 }}>
                    ...
                    <span style={{ fontSize: 10, color: COLORS.gray400 }}>{a.time}</span>
                  </div>
                </div>
              );
            })}
        ...
        <CardBox>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: GAP.sm }}>
            {ANNOUNCEMENTS.map((a, i, arr) => (
              <div key={a.id} style={{ display: "flex", gap: GAP.sm, paddingBottom: i < arr.length - 1 ? GAP.sm : 0, borderBottom: i < arr.length - 1 ? `1px solid ${COLORS.borderLight}` : "none" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: GAP.xs }}>
                    <p style={{ margin: 0, fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.semibold, color: COLORS.textLight, lineHeight: 1.3 }}>{a.title}</p>
                  ...
                  <p style={{ margin: "3px 0 0", fontSize: FONT_SIZE.xs, color: COLORS.textMutedLight, display: "flex", alignItems: "center", gap: 4 }}>
        ...
        <CardBox>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: GAP.sm }}>
            {UPCOMING_EVENTS.map((e, i, arr) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: GAP.sm, paddingBottom: i < arr.length - 1 ? GAP.sm : 0, borderBottom: i < arr.length - 1 ? `1px solid ${COLORS.borderLight}` : "none" }}>
                ...
                <p style={{ margin: 0, fontSize: FONT_SIZE.sm, color: COLORS.textLight, lineHeight: 1.3 }}>{e.label}</p>
              </div>
            ))}
          </div>
        </CardBox>
