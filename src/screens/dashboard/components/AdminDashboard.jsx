...
import { SimpleGrid, Box, Group, Text, Avatar, Stack, ThemeIcon, Loader, Center } from "@mantine/core";
import {
...
export const AdminDashboard = ({ employees: empProp = [] }) => {
  ...
  // Leave summary donut — use dept distribution as fallback if no leave data
  const deptDonutData = depts.slice(0, 5).map((d, i) => ({ name: d.name, value: d.count, color: PIE_COLORS[i % PIE_COLORS.length] }));
  const deptMapFallback = {};
  employees.forEach((e) => { const d = e.department || "Other"; deptMapFallback[d] = (deptMapFallback[d] || 0) + 1; });
  const donutData = deptDonutData.length
    ? deptDonutData
    : Object.entries(deptMapFallback).slice(0, 5).map(([name, value], i) => ({ name, value, color: PIE_COLORS[i % PIE_COLORS.length] }));
  ...
  return (
    <Box>
      ...
      <SimpleGrid cols={{ base: 1, sm: 2, lg: 5 }} spacing="md" mb="md">
        <KpiCard icon={IconUsers}          label="Total Employees"   value={total}                 sub={`${deptCount} departments`}     trend="3.2%"  up color="blue"   spark={ramp(total)} />
        ...
        <KpiCard icon={IconUserMinus}      label="Absent Today"      value={absent}                sub="Not checked in"                 trend="1.2%"  up={false} color="orange" spark={ramp(absent)} />
        <KpiCard icon={IconClock}          label="Pending Approvals" value={pendingLeaves}         sub="Awaiting HR review"             trend="2.3%"  up={false} color="red"    spark={ramp(pendingLeaves)} />
        <KpiCard icon={IconWallet}         label="Monthly Payroll"   value={fmtMoney(totalSalary)} sub={`Avg ${fmtMoney(avgSalary)}`}   trend="4.0%"  up color="violet" spark={payrollSpark} />
      </SimpleGrid>

      {/* ── Row 2: Attendance | Dept donut | Quick Actions ── */}
            ...
            : (
              <ResponsiveContainer width="100%" height={240}>
                <AreaChart data={attendSeries} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
                  <defs>
                    <linearGradient id="adGradPresent" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#4f46e5" stopOpacity={0.35} /><stop offset="95%" stopColor="#4f46e5" stopOpacity={0.02} /></linearGradient>
                    <linearGradient id="adGradAbsent"  x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#fa5252" stopOpacity={0.25} /><stop offset="95%" stopColor="#fa5252" stopOpacity={0.02} /></linearGradient>
                    <linearGradient id="adGradLeave"   x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#fab005" stopOpacity={0.25} /><stop offset="95%" stopColor="#fab005" stopOpacity={0.02} /></linearGradient>
                  </defs>
                  ...
                  <Area type="monotone" dataKey="Present"  stroke="#4f46e5" strokeWidth={2.5} fill="url(#adGradPresent)" />
                  <Area type="monotone" dataKey="Absent"   stroke="#fa5252" strokeWidth={2}   fill="url(#adGradAbsent)" />
                  <Area type="monotone" dataKey="On Leave" stroke="#fab005" strokeWidth={2}   fill="url(#adGradLeave)" />
                </AreaChart>
              </ResponsiveContainer>
            )}
        </PanelCard>
              ...
              <Group align="center" gap="lg" wrap="nowrap">
                <Box style={{ position: "relative", width: 150, height: 150, flexShrink: 0 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={donutData} dataKey="value" cx="50%" cy="50%" innerRadius={48} outerRadius={70} paddingAngle={3}>
