...
import { SimpleGrid, Box, Group, Text, Loader, Center, Card, Avatar, Stack, ThemeIcon, Badge } from "@mantine/core";
import {
...
import {
  ...
  IconUserPlus, IconCalendarStats, IconClipboardCheck, IconSpeakerphone,
  IconChevronRight, IconCake, IconArrowUpRight, IconArrowDownRight, IconBolt,
...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <Box p="xs" style={{ background: "var(--mantine-color-body)", border: "1px solid var(--mantine-color-default-border)", borderRadius: 8, boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}>
      ...
      {payload.map((p) => <Text key={p.dataKey || p.name} fz="xs" c={p.color}>{p.name}: <Text span fw={700}>{p.value}</Text></Text>)}
    </Box>
  );
};

// hex for sparkline strokes by Mantine color name
const SPARK_HEX = { blue: "#3b82f6", green: "#10b981", orange: "#f59e0b", violet: "#7c3aed", red: "#ef4444", grape: "#be4bdb", teal: "#14b8a6" };

// ── Premium KPI card: colored icon tile + trend badge + sparkline + hover lift ──
function KpiCard({ icon: Icon, label, value, trend, up, sub, color, spark = [] }) {
          ...
          <Box style={{ width: 72, height: 28, flexShrink: 0 }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={sparkData} margin={{ top: 2, right: 0, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="v" stroke={stroke} strokeWidth={1.8} fill={`url(#spark-${color})`} dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </Box>
        )}
...
function PanelCard({ title, sub, action, children, ...rest }) {
  return (
    <Card withBorder radius="lg" p="lg" {...rest}>
