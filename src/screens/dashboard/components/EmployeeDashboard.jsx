...
// ── Compact empty-state (icon badge + message) ─────────────────────────────────
const EmptyMini = ({ icon: Icon, message, sub, size = "sm" }) => (
  <Stack
    align="center"
    justify="center"
    gap="xs"
    py={size === "sm" ? "lg" : "xl"}
    style={{ minHeight: size === "sm" ? 132 : 176 }}
  >
    <Box
      style={{
        width: size === "sm" ? 52 : 64,
        height: size === "sm" ? 52 : 64,
        borderRadius: "50%",
    ...
    >
      <Icon size={size === "sm" ? 24 : 30} color="#94a3b8" stroke={1.5} />
    </Box>
    <Text c="dimmed" fz="sm" fw={600} mt={4}>
      {message}
    </Text>
    {sub && (
      <Text c="dimmed" fz="xs" ta="center" maw={220}>
        {sub}
      </Text>
    )}
  </Stack>
...
export const EmployeeDashboard = ({ user }) => {
  ...
  const attendPct =
    records.length > 0 ? Math.round((presentDays / records.length) * 100) : 0;
  ...
  const birthdays = allEmployees.filter(
    (e) => e.dob && new Date(e.dob).getMonth() === thisMonth,
  );
  const anniversaries = allEmployees.filter((e) => {
    if (!e.joinDate) return false;
    const j = new Date(e.joinDate);
    return (
      j.getMonth() === thisMonth &&
      new Date().getFullYear() - j.getFullYear() > 0
    );
  });
  ...
  return (
    <Box>
      ...
      <Paper
        ...
        style={{
          background:
            "linear-gradient(135deg, #0f172a 0%, #1e3a8a 45%, #1d4ed8 75%, #2563eb 100%)",
          border: "none",
          overflow: "hidden",
          position: "relative",
        }}
                ...
                >
                  <Box
                    style={{
                      ...
                      height: 8,
                      borderRadius: "50%",
                      background: statusColor(todayStatus),
                      flexShrink: 0,
                    }}
                  />
                  <Text fz="xs" c="rgba(255,255,255,0.9)" fw={600}>
                    Today: {todayStatus}
                    {todayRec?.checkIn
                      ? ` · In ${new Date(todayRec.checkIn).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}`
                      : ""}
                    {todayRec?.checkOut
                      ? ` → Out ${new Date(todayRec.checkOut).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}`
                      : ""}
                  </Text>
                </Box>
              </Stack>
      ...
      <SimpleGrid cols={{ base: 2, sm: 2, lg: 4 }} spacing="md" mb="xl">
        <KpiCard
          ...
          label="Attendance"
          value={`${attendPct}%`}
          sub={`${presentDays} / ${records.length} days`}
          trend={`${attendPct}%`}
          up={attendPct >= 75}
        ...
        <KpiCard
          ...
          color="violet"
          label="Net Salary"
          value={payslip?.net ? fmt(payslip.net) : "—"}
          sub={payslip?.month || "Latest payslip"}
      ...
      <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md" mb="md">
        ...
        <PanelCard
          title="My Attendance"
          sub="This month's summary"
          action={<ViewAll navigate={navigate} to="/attendance" />}
        >
          ...
          {records.length === 0 ? (
            <EmptyMini icon={IconClock} message="No records yet" />
          ) : (
              ...
              >
                ...
                >
                  ...
                  <Text
                    fz="xs"
                    fw={600}
                    style={{ color: statusText(r.status) }}
                  >
                    {r.status}
                  </Text>
                </Box>
        ...
        <PanelCard
          ...
          sub="Current year allocation"
          action={<ViewAll navigate={navigate} to="/leave" />}
        >
          {balance.length === 0 ? (
            <EmptyMini icon={IconCalendarOff} message="No leave balance data" />
          ) : (
            balance.map((b) => {
              ...
              {leaves.slice(0, 3).map((l, i, arr) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Badge color={sc} variant="light" size="sm">
                      {l.status}
                    </Badge>
                  </Group>
                );
      ...
      <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md" mb="md">
        ...
        <PanelCard
          ...
          sub={payslip?.month || "—"}
          action={<ViewAll navigate={navigate} to="/my-payslips" />}
        >
          {payslip &&
          [payslip.gross, payslip.deductions, payslip.net].every((n) =>
            ...
            <>
              <SimpleGrid cols={3} spacing="sm" mb="md">
                {[
                  {
                    label: "Gross",
                    value: fmt(payslip.gross),
                    color: "#3b82f6",
                    bg: "#eff6ff",
                  },
                  {
                    label: "Deductions",
                    value: `-${fmt(payslip.deductions)}`,
                    color: "#ef4444",
                    bg: "#fef2f2",
                  },
                  {
                    label: "Net Pay",
                    value: fmt(payslip.net),
                    color: "#22c55e",
                    bg: "#f0fdf4",
                  },
                ].map((s) => (
              ...
              <Group>
                <Badge
                  color={payslip.status === "Paid" ? "green" : "yellow"}
                  variant="light"
            ...
            <EmptyMini
              icon={IconWallet}
              message="No payslip available yet"
              sub="Your payslip will show up here once payroll is processed."
              size="lg"
            />
          )}
        </PanelCard>
        ...
        <PanelCard
          title="Announcements"
          sub="Latest company notices"
          action={<ViewAll navigate={navigate} to="/announcements" />}
        >
          {announcements.length === 0 ? (
            <EmptyMini icon={IconAlertCircle} message="No announcements" size="lg" />
          ) : (
            announcements.slice(0, 4).map((a, i, arr) => {
              const grad =
                PRIORITY_GRAD[a.priority] ||
              ...
              return (
                ...
                >
                  ...
                  <Badge
                    size="xs"
                    style={{
                      background: a.priority === "High" ? "#fef2f2" : "#f0fdf4",
                      color: a.priority === "High" ? "#dc2626" : "#16a34a",
                      flexShrink: 0,
                    }}
                  >
                    {a.priority}
      ...
      <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md">
        ...
        <PanelCard title="🎂 Birthdays" sub={`${birthdays.length} this month`}>
          {birthdays.length === 0 ? (
            <EmptyMini icon={IconCake} message="No birthdays this month" />
          ) : (
            birthdays.slice(0, 4).map((e) => (
              ...
              >
                ...
                <Box style={{ minWidth: 0 }}>
                  ...
                  <Text fz="xs" c="dimmed">
                    {new Date(e.dob).toLocaleDateString("en-IN", {
                      day: "2-digit",
                      month: "long",
                    })}
                  </Text>
        ...
        <PanelCard
          title="⭐ Anniversaries"
          sub={`${anniversaries.length} this month`}
        >
          {anniversaries.length === 0 ? (
            <EmptyMini icon={IconStar} message="No anniversaries this month" />
          ) : (
            anniversaries.slice(0, 4).map((e) => {
              const years =
                new Date().getFullYear() - new Date(e.joinDate).getFullYear();
              return (
                <Group
                  key={e.id}
                  gap="sm"
        ...
        <PanelCard title="📅 Upcoming" sub="Events & holidays">
          {events.length === 0 ? (
            <EmptyMini icon={IconCalendarEvent} message="No upcoming events" />
          ) : (
            events.slice(0, 4).map((e, i, arr) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <Text fz={8} fw={800} c="blue" tt="uppercase" lh={1}>
                      {d.toLocaleDateString("en-IN", { month: "short" })}
                    </Text>
                    <Text fz="md" fw={800} c="blue" lh={1.1}>
                      {d.getDate()}
                    </Text>
                  </Box>
                  <Box style={{ minWidth: 0 }}>
