  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderRadius: 14, padding: "20px 24px", boxShadow: "0 1px 3px rgba(0,0,0,0.07)" }}>
        <h2 style={{ fontSize: 16, fontWeight: 700, color: "#0f172a", margin: "0 0 16px" }}>Attendance List</h2>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: "2px solid #e2e8f0" }}>
              {["Employee","Department","Check In","Check Out","Date","Status","Action"].map(h => (
          ...
          <tbody>
            {attendance.map((item) => (
              <tr key={item._id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                <td style={{ padding: "13px 0", fontSize: 14, fontWeight: 500, color: "#0f172a" }}>{item.employee}</td>
                ...
                <td style={{ padding: "13px 0", fontSize: 14, color: "#334155" }}>{item.date}</td>
                <td style={{ padding: "13px 0" }}>
                  <span style={{
                    display: "inline-block", padding: "4px 12px", borderRadius: 999,
                    fontSize: 12, fontWeight: 600,
                    background: item.status === "Present" ? "#dcfce7" : "#fee2e2",
                    color: item.status === "Present" ? "#15803d" : "#b91c1c",
                  }}>{item.status}</span>
                </td>
                <td style={{ padding: "13px 0" }}>
